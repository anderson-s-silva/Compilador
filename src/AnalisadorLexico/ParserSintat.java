...
public class ParserSintat {
	...
	private ArrayList<String> decVar = new ArrayList<String>();
	private ArrayList<Token> decTipo = new ArrayList<Token>();
	private ArrayList<String> usedVar = new ArrayList<String>();
	private ArrayList<Token> checkType = new ArrayList<Token>();
	
	public ParserSintat() {}

	public void parser(ArrayList<Token> arraytoken) {
		ArrayList<Token> arrayCompare = new ArrayList<Token>();//dois arrays para serem carimbados por parte do arraytoken e facilitar a verificacao
		ArrayList<Token> arrayListaComandos = new ArrayList<Token>();

		int aux = 0;
		if(arraytoken.get(0).getnome() == TipoToken.Delim) {//verifica os primeiros 3 itens que devem ser respectivamente delim, pcdec e delim
		...
		for(int i = 2; i < arraytoken.size(); i++) { //laço que procura o proximo delim seguido de pcprog
			if(arraytoken.get(i).getnome() == TipoToken.Delim && arraytoken.get(i+1).getnome() == TipoToken.PCProg) {
				...
				aux = i;//armazeno o indice do token Delim
				for(int j = 2; j < aux; j++) arrayCompare.add(arraytoken.get(j)); // adiciono ao arraycompare tudo entre DEC e delim, no caso, adiciono a lista de declaracoes
			}			
		}
		if(aux +1 == arraytoken.size()) {//não encontrei nada apos o DEC
			System.out.println("ERROR: sem LISTACOMANDOS na linha " + Integer.toString(arrayListaComandos.get(0).getlinha()));
			System.exit(1);
		}
		for(int i = aux+2; i < arraytoken.size(); i++) arrayListaComandos.add(arraytoken.get(i));//adiciono ao arrayListaComandos a lista de comandos
		...
		if(arrayDeclaracao.get(0).getnome() == TipoToken.Var) {
			if(arrayDeclaracao.get(1).getnome() == TipoToken.Delim) {
				if(arrayDeclaracao.get(2).getnome() == TipoToken.PCReal || arrayDeclaracao.get(2).getnome() == TipoToken.PCInt) {
					decVar.add(arrayDeclaracao.get(0).getlex());//var
					decTipo.add(arrayDeclaracao.get(2));//tipovar
					retorno = true;
				}else {
	...
	public boolean comandoAtrib(ArrayList<Token> arrayListaComandos) {
		...
		if(arrayListaComandos.get(0).getnome() == TipoToken.Var) {
			usedVar.add(arrayListaComandos.get(0).getlex());
			checkType.add(arrayListaComandos.get(0));
	...
	public boolean comandoEntrada(ArrayList<Token> arrayListaComandos) {
		...
		if(arrayListaComandos.get(0).getnome() == TipoToken.PCLer) {
			...
			if(arrayListaComandos.get(0).getnome() == TipoToken.Var) {
				...
				System.out.println("ERROR: Esperado Token Var na linha " + Integer.toString(arrayListaComandos.get(0).getlinha()));
				System.exit(1);
			}
		}
		return retorno;
	...
	public boolean comandoSaida(ArrayList<Token> arrayListaComandos) {
		...
		if(arrayListaComandos.get(0).getnome() == TipoToken.PCImprimir) {
			...
			if(arrayListaComandos.get(0).getnome() == TipoToken.Var || arrayListaComandos.get(0).getnome() == TipoToken.Cadeia) {
				if(arrayListaComandos.get(0).getnome() == TipoToken.Var) usedVar.add(arrayListaComandos.get(0).getlex());
				arrayListaComandos.remove(0);
				retorno = true;
			}
		}
	...
	public boolean termoArit(ArrayList<Token> arrayListaExpressao) {
		...
		if(fatorArit(arrayListaExpressao)) {
			if(termoAritL(arrayListaExpressao)) retorno = true;
		}
		return retorno;
	}
	...
	public boolean fatorArit(ArrayList<Token> arrayListaExpressao) {
		...
		if(arrayListaExpressao.get(0).getnome() == TipoToken.Var) {
			usedVar.add(arrayListaExpressao.get(0).getlex());
			checkType.add(arrayListaExpressao.get(0));
			...
			while(!arrayListaExpressao.isEmpty() && arrayListaExpressao.get(0).getlinha() == linha) {
				if(arrayListaExpressao.get(0).getnome() != TipoToken.FechaPar) {
					...
					if(arrayListaExpressao.get(0).getnome() == TipoToken.Var || arrayListaExpressao.get(0).getnome() == TipoToken.NumInt || arrayListaExpressao.get(0).getnome() == TipoToken.NumReal){
						checkType.add(arrayListaExpressao.get(0));
					}
					arrayListaExpressao.remove(0);
				}else {
	...
	public ArrayList<String> getusedVar(){
		return this.usedVar;
	}
	public ArrayList<String> getdecVar(){
		return this.decVar;
	}
	public ArrayList<Token> getdecTipo(){
		return this.decTipo;
	}
	public ArrayList<Token> getCheckType(){
